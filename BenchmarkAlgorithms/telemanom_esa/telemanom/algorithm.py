...
class AlgorithmArgs(argparse.Namespace):
    ...
    def ts(self) -> np.ndarray:
        ...
        # ✅ keep only anomaly cols that actually exist
        existing_anom = [c for c in all_used_anomaly_columns if c in dataset.columns]

        # keep only the channels + existing anomaly cols
        dataset = dataset.loc[:, all_used_channels + existing_anom]

        # keep data columns list aligned with just the channels
        data_columns = all_used_channels

        self._map_channels_to_indices(data_columns)

        if self.executionType == "train":
            return self._prepare_training_data(dataset, existing_anom)
        else:
            # ✅ safe even if there are zero anomaly cols
            cut = len(existing_anom)
            arr = dataset.values.astype(np.float32) if cut == 0 else dataset.values[:, :-cut].astype(np.float32)
            dataset = np.expand_dims(arr, axis=0)
    ...
    def _read_dataset(self):
        ...
        # edited: Safe even when there are 0 anomaly columns
        data_columns = [c for c in columns if c not in anomaly_columns]
        
        dtypes = {col: np.float32 for col in data_columns}
        dtypes.update({col: np.uint8 for col in anomaly_columns})
        dataset = pd.read_csv(self.dataInput, index_col="timestamp", parse_dates=["timestamp"], dtype=dtypes) #edited too
